...
def main(argv):
    ...
    assert os.path.exists(config_path), f"Config path `{config_path}` does not exists in {os.getcwd()}"
    assert os.path.exists(csv_path), f"CSV path `{csv_path}` does not exists in {os.getcwd()}"
