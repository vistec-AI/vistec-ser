...
def load_waveform(
    ...
    audio_rate = tf.cast(audio_tensor.rate, tf.int64)
    audio_tensor = tf.reduce_mean(tf.cast(audio_tensor.to_tensor(), tf.float32), axis=-1)
    waveform = tf.divide(audio_tensor, normalize_factor)
